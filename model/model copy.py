...
class DDPM(BaseModel):
    ...
    def test(self, continous=False):
        ...
        with torch.no_grad():
            ...
            self.data['noise1'] = self.data['SR']-self.IP

            if isinstance(self.netG, nn.DataParallel):
                self.pred_noise = self.netG.module.super_resolution(
                    self.data['noise1'],continous)
            else:
                self.pred_noise = self.netG.super_resolution(
                    self.data['noise1'], continous)
            self.clean2=self.data['SR']-self.pred_noise
